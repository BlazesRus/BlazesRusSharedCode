...
namespace BlazesRusCode
{
    ...
    public:
        ...
        void SwitchOpToVal(FormData& FormCopy, FormElement& OpVal, int OpKey, AltDec Value)
        {
            ...
            for (int opIndex = 0; opIndex < 11; ++opIndex)
            {
                ...
                for (IntVector::iterator CurrentVal = OpOrderElement.begin(), LastVal = OpOrderElement.end(); CurrentVal != LastVal; ++CurrentVal)
                {
                    ...
                    else if (opIndex != 0 || (OpIterator->second.ElementCat != FormulaElementType::Sqrt && OpIterator->second.ElementCat != FormulaElementType::LN && OpIterator->second.ElementCat != FormulaElementType::LOGTEN))
                    {
                        ...
                        case FormulaElementType::Formula:
                        {
                            ...
                            if (targetSegmentRef.size() == 1)
                            {
                                ...
                                if (targetElem->second.ElementCat != FormulaElementType::Variable)
                                    leftValue = targetElem->second.ElementCat == FormulaElementType::trueVal ? AltDec::One : (targetElem->second.ElementCat == FormulaElementType::falseVal ? AltDec::Zero : targetSegmentRef.NumMap[targetElem->first]);
                                else
                        ...
                        case FormulaElementType::trueVal:
                            leftValue = AltDec::One;
                        case FormulaElementType::falseVal:
                            leftValue = AltDec::Zero;
                        default:
                            continue; break;
                        }
                    }
                    ...
                    case FormulaElementType::Formula:
                    {
                        ...
                        if (targetSegmentRef.size() == 1)
                        {
                            FormData::iterator targetElem = targetSegmentRef.begin();
                            if (targetElem->second.ElementCat != FormulaElementType::Variable)
                                rightValue = targetElem->second.ElementCat == FormulaElementType::trueVal ? AltDec::One : (targetElem->second.ElementCat == FormulaElementType::falseVal ? AltDec::Zero : targetSegmentRef.NumMap[targetElem->first]);
                            else
                                continue;//Ignore operation with unknown variable value
                        }
                        else
                    ...
                    case FormulaElementType::trueVal:
                        rightValue = AltDec::One;
                    case FormulaElementType::falseVal:
                        rightValue = AltDec::Zero;
                    default:
                        continue; break;
                    }
                    switch (opIndex)
                    ...
                    case 1://Remove operator in this case and change right side value
                        ...
                        case FormulaElementType::Not:
                            ...
                            {
                                FormDRef.at(RightVal->first).ElementCat = rightValue == AltDec::Zero ? FormulaElementType::trueVal : FormulaElementType::falseVal;
                            }
                            else//Assuming is number
                            {
                                if (rightValue == AltDec::Zero)//Zero is false otherwise count as if it was true
                                    FormDRef.NumMap[RightVal->first] = AltDec::One;//SwitchOpToBoolVal(FormDRef, FormDRef[*CurrentVal], true);
                                else
                                    FormDRef.NumMap[RightVal->first] = AltDec::Zero;//SwitchOpToBoolVal(FormDRef, FormDRef[*CurrentVal], false);
                            }
                            break;
        ...
        void ReplaceVariablesWithRefValues(ReferenceMap ElementValues, size_t FormIndex = 0)
        {
            ...
            for (FormData::iterator CurrentVal = FormDRef.begin(), LastVal = FormDRef.end(); CurrentVal != LastVal; ++CurrentVal)
            {
                ...
                else if (CurrentVal->second.ElementCat == FormulaElementType::Variable)//Swap Variable with values
                {
                    ...
                    tsl::ordered_map<std::string, AltDec&>::iterator KeyedElemVal = ElementValues.find(CurString);
        ...
        /// </summary>
        /// <param name="ElementValues">The element values.</param>
        /// <returns>BlazesRusCode.AltFormula</returns>
        AltFormula EvaluateRefToSimplifiedForm(ReferenceMap ElementValues)
        {
            AltFormula FormCopy = *this;//Duplicate values so can erase parts when calculating
        ...
        /// <param name="ElementValues">The element values.</param>
        /// <returns>BlazesRusCode.AltFormula</returns>
        AltFormula EvaluateToSimplifiedForm(ValueMap ElementValues)
        {
            AltFormula FormCopy = *this;//Duplicate values so can erase parts when calculating
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AltFormula" /> class.
        /// </summary>
        /// <param name="ElemValue">The elem value to read in order to create formula data.</param>
        AltFormula(std::string& ElemValue)
        {
            //0 = ???
            //1 = Operator
            //3 = Variable Value
...
*/
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AltFormula" /> class from StringCopy instead of reference.
        /// </summary>
        /// <param name="ElemValue">The elem value to read in order to create formula data.</param>
        AltFormula(std::string ElemValue, bool BlankVar) : AltFormula(ElemValue) {}

        /// <summary>
        /// Initializes a new instance of the <see cref="AltFormula" /> class.(fix for initializing without copying from a string value set)
        /// </summary>
        /// <param name="ElemValue">The elem value to read in order to create formula data.</param>
        AltFormula(const char* strVal) : AltFormula(std::string(strVal), true) {}
    };
}