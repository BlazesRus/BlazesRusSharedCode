...
namespace CSharpGlobalCode.GlobalCode_NodeTrees
{
	...
	{
		private string CurrentHavokClass = "";
		private string CurrentActionDataSection_01 = "";
	};

	internal class TagNodeTreeSharedData
	{
	...
	public class Node
	{
		...
		private string InternalNameOfTagClosed = "";
		...
		//Additional Tag Args
		private XMLOptionList AdditionTagOptions;

		private bool SelfContainedTag = false;
		private bool XMLVersionTag = false;
		private bool UsingAlternativeTagContentStorage = false;
		...
		private void DetectTagContentTypesWithin(NodeTreeType NodeTreeTarget)
		{
		...
		// Qualifier:
		//************************************
		private void DetectTagContentTypeV2(OptimizationData& SharedData)
		{
		}
		...
		private void DetectTagContentTypesWithinV2(NodeTreeType* NodeTreeTarget, OptimizationData& SharedData)
		{
			string TargetNameTemp;
			Node TargetNode;
		...
		//************************************
		private void GenerateHTMLDoc(StringList OutputBuffer, int OutputLvl, byte GenerationOptions = 0)
		{
		}
		...
		private void GenerateHTMLDocWithin(NodeTreeType* NodeTreeTarget, StringList OutputBuffer, int OutputLvl, byte GenerationOptions = 0)
		{
			string TargetNameTemp;
			Node TargetNode;
			...
			for (int Index = 0; Index < ChildListSize; ++Index)
			{
				...
				TargetNode = NodeTreeTarget->private GetNodePointerFromInternalName(NodeName);

				if (TargetNode != nullptr)
				{
					TargetNode->private GenerateHTMLDocWithin(NodeTreeTarget, OutputBuffer, OutputLvl, GenerationOptions);
				}
			}
		}
	}
