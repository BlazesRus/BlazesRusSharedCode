...
long long int StringFunctions::NumberOfPlaces(long long int Value)
{
...
// Parameter: int Temp
//************************************
char StringFunctions::DigitAsChar(int Temp)
{
	char Value = '0';
	if(Temp == 0) { Value = '0'; }
	else if(Temp == 1) { Value = '1'; }
...
long long int StringFunctions::ReadXIntFromString(std::string TempString)
{
...
std::string StringFunctions::XIntToStringConversion(long long int TempValue)
{
	string TempString = "";
	bool IsNegative = (TempValue < 0) ? true : false;
	if(IsNegative)
