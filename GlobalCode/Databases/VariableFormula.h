...
class VarFormElement;

template<typename VarType>
class DLL_API VariableFormula
{
public:
	class DLL_API VarFormElement
	{
	public:
		/// <summary>
		/// The element type (Other values refer to variable names)
		/// @ = Formula Val; # = Number Value; ? = TernaryOperator(Not Supported Yet)
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="VarFormElement"/> class. (Defaults as boolean with true value)
		/// </summary>
		VarFormElement()
		{
			ElementType = "true";
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="VarFormElement"/> class.
		/// </summary>
		/// <param name="Value">The value.</param>
		/// <param name="index">The index.</param>
		VarFormElement(std::string Value, int index = 0)
		{
			ElementType = Value;
			Index = index;
		}
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="VarFormElement"/> class for PostFix OpValue.
		/// </summary>
		/// <param name="Value">The value.</param>
		/// <param name="opBuffer">The operator value.</param>
		VarFormElement(std::string Value, std::string opBuffer)
		{
	...
	class DLL_API VariableFormData : public tsl::ordered_map<int, VarTypeFormElement>
	{
	private:
	...
	public:
		...
		std::unordered_map<int, VarType> AltMap;
		
		std::unordered_map<int, std::string> VariableMap;
		std::unordered_map<std::string, VarType> VariableStorageMap;
	private:
		/// <summary>
		/// The next index
		/// </summary>
		...
		unsigned int Add(int Key, VarTypeFormElement Value)
		{
	...
	protected:
		...
		int Add(VarTypeFormElement Value)
		{
			int IndexPos;
	...
	public:
		...
		{
			VarTypeFormElement& KeyedVal = this->at(Key);
		...
		void SwitchOpToVal(ExprFormElement& OpVal, VariableFormData::iterator& LeftIterator, VariableFormData::iterator& RightIterator, bool Value)
		{
			if (Value) { OpVal.ElementType = "true"; }
			else { OpVal.ElementType = "false"; }

			if (LeftIterator->second.ElementType == "#"){ AltMap.erase(LeftIterator->first); }
			if (LeftIterator->second.ElementType == "#"){ AltMap.erase(RightIterator->first); }
		...
		void SwitchOpToVal(VarTypeFormElement& OpVal, int OpKey, VariableFormData::iterator& RightIterator, VarType Value)
		{
			AltMap.insert_or_assign(OpKey, Value);
			OpVal.ElementType = "#";
			
			if (RightIterator->second.ElementType == "#"){ AltMap.erase(RightIterator->first); }
		...
		void SwitchOpToVal(VarTypeFormElement& OpVal, int OpKey, VariableFormData::iterator& LeftIterator, VariableFormData::iterator& RightIterator, VarType Value)
		{
			AltMap.insert_or_assign(OpKey, Value);
			OpVal.ElementType = "#";
		...
		{
			LastAdded = Add(ExprFormElement("#"));
			AltMap.insert_or_assign(LastAdded, Value);
		}
		...
		{
			LastAdded = Add(VarTypeFormElement(Value));
			VariableMap.insert_or_assign(LastAdded, Value);
			//VariableStorageMap.insert_or_assign(Value, VarType::Undefined);
		}
		...
		{
			LastAdded = Add(VarTypeFormElement("$" + Value, Value.front() == '+' ? 0 : 1));
			OpOrderMap[1].push_back(LastAdded);
		}
		...
		void ChangeLastToPostfixOp(std::string Value, size_t FormIndex)
		{
			...
			LastAdded = Add(VarTypeFormElement(strBuffer, Value));
		}
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="VariableFormData"/> class.
		/// </summary>
		VariableFormData()
		...
		/// <summary>
		/// Finalizes an instance of the <see cref="VariableFormData"/> class.
		/// </summary>
		~VariableFormData() {}
	};
private:
    using IntVector = std::vector<int>;
    /// <summary>
    /// Prevents a default instance of the <see cref="VariableFormula"/> class from being created.
    /// </summary>
    VariableFormula() {}
...
public:
        ...
        for (int opIndex = 0; opIndex < 12; ++opIndex)
        {
            ...
            for (IntVector::iterator CurrentVal = OpOrderElement.begin(), LastVal = OpOrderElement.end(); CurrentVal != LastVal; ++CurrentVal)
            {
                ...
                case 1://(++ PreFix, -- PreFix Not Supported), ! (Unary plus and minus not supported yet)
					...
					//and switching numbers to either zero or one(later)
					...
					else if(OpIterator->second.ElementType == "-")//Only applies to numbers
					{
						...
						FormCopy.NumMap[RightVal->first].second
					}
                    //else//switching numbers to either zero or one
                    //{
                    //}
                    break;
                ...
                case 3://Addition and subtraction
                    ...
                    //	break;
                case 4://<,<=, >, >=
                    leftValue = LeftVal->second.ElementType == "true" ? 1 : (LeftVal->second.ElementType == "false" ? 0 : FormCopy.NumMap[LeftVal->first]);//(LeftVal->second.ElementType == "#" ? FormCopy.NumMap[LeftVal->first]: 0)
                    rightValue = RightVal->second.ElementType == "true" ? 1 : (RightVal->second.ElementType == "false" ? 0 : FormCopy.NumMap[RightVal->first]);
    ...
    VariableFormula(std::string ElemValue)
    {
        //0 = ???
        //1 = Operator
        //3 = Variable Value
